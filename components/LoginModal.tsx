...
import { X, Mail, Lock, LogIn, Heart, Stethoscope, User, ArrowRight, Loader2, ShieldCheck, Smartphone, Shield, ArrowLeft } from 'lucide-react';
...
const LoginModal: React.FC<LoginModalProps> = ({ onClose, onLogin, initialRole, initialMode = 'login', language }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-xl p-4 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
       ...
       <div className="relative w-full max-w-5xl h-[650px] bg-white dark:bg-slate-950 rounded-[3rem] shadow-2xl flex overflow-hidden border border-white/10 dark:border-slate-800">
          <button 
             onClick={onClose}
             className={`absolute top-6 p-2 rounded-full bg-slate-100/50 hover:bg-slate-200 dark:bg-slate-800/50 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-300 transition-colors z-50 backdrop-blur-md ${language === 'ar' ? 'left-6' : 'right-6'}`}
           >
             <X size={24} />
           </button>
                ...
                <div className="h-full flex flex-col justify-center animate-fade-in-up">
                    ...
                    <div className="space-y-4 max-w-md mx-auto w-full mt-8">
                        <button onClick={() => { setSelectedRole(UserRole.CLIENT); setIsRegister(false); setView('login'); }} className="group w-full bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 p-5 rounded-[2rem] flex items-center gap-5 transition-all hover:shadow-xl hover:-translate-y-1">
                            ...
                            {language === 'ar' ? <ArrowLeft className="text-slate-300 group-hover:text-emerald-500 transition-colors" /> : <ArrowRight className="text-slate-300 group-hover:text-emerald-500 transition-colors" />}
                        </button>
             ...
             {view === 'login' && (
                <div className="h-full flex flex-col justify-center animate-fade-in-up max-w-sm mx-auto w-full">
                    <button onClick={() => setView('role')} className="text-xs font-bold text-slate-400 hover:text-slate-700 dark:hover:text-white mb-8 flex items-center gap-2 transition-colors w-fit">
                        {language === 'ar' ? <ArrowRight size={16} /> : <ArrowLeft size={16} />} {t.changeRole}
                    </button>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 mt-6">
                        {isRegister && !isAdmin && (
                            <div className="relative group">
                                <User className={`absolute top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-500 transition-colors ${language === 'ar' ? 'right-4' : 'left-4'}`} size={20} />
                                <input type="text" placeholder={t.fullName} value={name} onChange={e => setName(e.target.value)} className={`w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl py-4 pr-4 pl-4 font-bold text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 dark:focus:border-emerald-500 transition-all placeholder:text-slate-400 ${language === 'ar' ? 'pr-12' : 'pl-12'}`} required />
                            </div>
                        )}

                        <div className="relative group">
                            <Mail className={`absolute top-1/2 -translate-y-1/2 text-slate-400 transition-colors ${isDoctor ? 'group-focus-within:text-blue-500' : isAdmin ? 'group-focus-within:text-slate-600' : 'group-focus-within:text-emerald-500'} ${language === 'ar' ? 'right-4' : 'left-4'}`} size={20} />
                            <input type="email" placeholder={t.email} value={email} onChange={e => setEmail(e.target.value)} className={`w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl py-4 pr-4 pl-4 font-bold text-slate-900 dark:text-white focus:outline-none transition-all placeholder:text-slate-400 ${isDoctor ? 'focus:border-blue-500' : isAdmin ? 'focus:border-slate-600' : 'focus:border-emerald-500'} ${language === 'ar' ? 'pr-12' : 'pl-12'}`} required />
                        </div>

                        <div className="relative group">
                            <Lock className={`absolute top-1/2 -translate-y-1/2 text-slate-400 transition-colors ${isDoctor ? 'group-focus-within:text-blue-500' : isAdmin ? 'group-focus-within:text-slate-600' : 'group-focus-within:text-emerald-500'} ${language === 'ar' ? 'right-4' : 'left-4'}`} size={20} />
                            <input type="password" placeholder={t.password} value={password} onChange={e => setPassword(e.target.value)} className={`w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl py-4 pr-4 pl-4 font-bold text-slate-900 dark:text-white focus:outline-none transition-all placeholder:text-slate-400 ${isDoctor ? 'focus:border-blue-500' : isAdmin ? 'focus:border-slate-600' : 'focus:border-emerald-500'} ${language === 'ar' ? 'pr-12' : 'pl-12'}`} required />
                        </div>
                            ...
                            <div className="space-y-4 pt-2 animate-fade-in">
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="relative">
                                        ...
                                        <div className={`absolute top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 ${language === 'ar' ? 'left-4' : 'right-4'}`}>▼</div>
                                    </div>
                                    <div className="relative">
                                        <Smartphone className={`absolute top-1/2 -translate-y-1/2 text-slate-400 ${language === 'ar' ? 'right-4' : 'left-4'}`} size={16} />
                                        <input type="text" placeholder={t.phone} value={phone} onChange={e => setPhone(e.target.value)} className={`w-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl py-4 pr-4 pl-4 font-bold text-xs text-slate-900 dark:text-white focus:border-blue-500 outline-none ${language === 'ar' ? 'pr-10' : 'pl-10'}`} required />
                                    </div>
                                </div>
             ...
             {(view === 'forgot' || view === 'verify' || view === 'reset') && (
                 <div className="h-full flex flex-col justify-center animate-fade-in-up max-w-sm mx-auto">
                     <button onClick={() => setView('login')} className="mb-6 text-slate-400 hover:text-slate-600 font-bold flex items-center gap-2">
                        {language === 'ar' ? <ArrowRight size={16}/> : <ArrowLeft size={16}/>} {tCommon.back}
                     </button>
