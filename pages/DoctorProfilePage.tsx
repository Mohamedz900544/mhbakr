...
const DoctorProfilePage: React.FC<DoctorProfilePageProps> = ({ doctor, setPage, isLoggedIn, onLoginReq, onBookAppointment, language }) => {
  ...
  // Mock Services Data
  const services = [
    { name: language === 'ar' ? 'كشف عام' : 'General Checkup', price: doctor.price, icon: Stethoscope },
    { name: language === 'ar' ? 'تطعيمات دورية' : 'Vaccination', price: 250, icon: Syringe },
    { name: language === 'ar' ? 'حلاقة وعناية' : 'Grooming', price: 300, icon: Scissors },
    { name: language === 'ar' ? 'جراحة صغرى' : 'Minor Surgery', price: 800, icon: ActivityIcon },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 pb-12 animate-fade-in transition-colors duration-300 font-sans relative">
      ...
      <div className="relative h-[350px] overflow-hidden">
          ...
          <img src="https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&q=80&w=1200" className="w-full h-full object-cover opacity-30 mix-blend-overlay" alt="Clinic Cover" />
                                ...
                                <AccordionItem id="hours" title={language === 'ar' ? "مواعيد العمل" : "Working Hours"} icon={Clock}>
                                    <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-3xl grid grid-cols-2 gap-6 text-sm border border-slate-100 dark:border-slate-700/50">
                                        <div className="flex flex-col gap-1 pb-2 border-b border-slate-200 dark:border-slate-700">
                                            <span className="text-slate-500 dark:text-slate-400 font-bold">Sat - Thu</span>
                                            <span className="font-black text-slate-900 dark:text-white text-lg">10:00 AM - 10:00 PM</span>
                                        </div>
                                        <div className="flex flex-col gap-1 pb-2 border-b border-slate-200 dark:border-slate-700">
                                            <span className="text-slate-500 dark:text-slate-400 font-bold">Fri</span>
                                            <span className="font-black text-emerald-500 text-lg">2:00 PM - 10:00 PM</span>
                                        </div>
                                    </div>
                                </AccordionItem>
                            </div>
