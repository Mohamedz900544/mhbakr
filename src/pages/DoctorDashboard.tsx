...
import { 
    ...
    Users, Settings, CalendarDays, Wallet, Star, CheckCircle, ArrowUpRight, Lock, 
    Plus, Trash2, Image as ImageIcon, Award, Clock3, Briefcase, Stethoscope
} from 'lucide-react';
...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ setPage, initialTab = 'overview', doctorProfile, appointments, onAction, onAddPrescription, onUpdateProfile, language }) => {
  ...
  const [editForm, setEditForm] = useState<Doctor>(doctorProfile);
  
  // Local state for adding items to lists
  ...
  const handleAddService = () => {
      ...
      const newService = { name: newServiceName, price: parseFloat(newServicePrice) };
      setEditForm(prev => ({
          ...prev,
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 pt-8">
                ...
                {activeTab === 'settings' && (
                    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in relative z-10 pb-20">
                        {/* Basic Info */}
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                            ...
                            <div className="space-y-6">
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-500 mb-2">Clinic Name / Dr Name</label>
                                        <input 
                                            type="text" 
                                            value={editForm.name} 
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-500 mb-2">Title (e.g. Senior Surgeon)</label>
                                        <input 
                                            type="text" 
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-500 mb-2">Base Checkup Price</label>
                                        <input 
                                            type="number" 
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-500 mb-2">Bio / About Clinic</label>
                                    <textarea 
                                        value={editForm.bio} 
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <Stethoscope size={20} className="text-emerald-500" /> Services & Fees
                            </h3>
                            <div className="grid grid-cols-[1fr_100px_auto] gap-2 mb-4">
                                <input 
                                    type="text" 
                                    placeholder="Service Name (e.g. Vaccination)" 
                                    value={newServiceName}
                                    onChange={(e) => setNewServiceName(e.target.value)}
                                    className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-3 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                />
                                <input 
                                    type="number" 
                                    placeholder="Price" 
                                    ...
                                    className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-3 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                />
                                <button onClick={handleAddService} className="bg-emerald-600 hover:bg-emerald-500 text-white p-3 rounded-xl transition-colors">
                                    <Plus size={20} />
                                </button>
                            </div>
                            <div className="space-y-2">
                                {editForm.services?.map((svc, i) => (
                                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700">
                                        <span className="text-sm font-bold text-gray-700 dark:text-gray-300">{svc.name}</span>
                                        <div className="flex items-center gap-4">
                                            <span className="font-black text-emerald-600 dark:text-emerald-400">{svc.price} {t.currency}</span>
                                            <button onClick={() => handleRemoveService(i)} className="text-red-400 hover:text-red-500 p-1">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </div>
                                ...
                                {(!editForm.services || editForm.services.length === 0) && (
                                    <p className="text-slate-400 text-sm italic text-center">No services added.</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <Clock3 size={20} className="text-orange-500" /> Working Hours
                            </h3>
                            <div className="grid md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input 
                                        type="time" 
                                        value={editForm.workingHours?.start || '10:00'} 
                                        onChange={(e) => setEditForm({...editForm, workingHours: { ...editForm.workingHours, start: e.target.value, end: editForm.workingHours?.end || '22:00' }})}
                                        className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input 
                                        type="time" 
                                        value={editForm.workingHours?.end || '22:00'} 
                                        onChange={(e) => setEditForm({...editForm, workingHours: { ...editForm.workingHours, start: editForm.workingHours?.start || '10:00', end: e.target.value }})}
                                        className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <Award size={20} className="text-blue-500" /> Qualifications
                            </h3>
                            <div className="flex gap-2 mb-4">
                                <input 
                                    type="text" 
                                    placeholder="Add qualification (e.g. PhD in Surgery)" 
                                    value={newQualification}
                                    onChange={(e) => setNewQualification(e.target.value)}
                                    className="flex-1 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-3 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                />
                                <button onClick={handleAddQualification} className="bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-xl transition-colors">
                                    <Plus size={20} />
                                </button>
                            </div>
                            <div className="space-y-2">
                                {editForm.qualifications?.map((q, i) => (
                                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700">
                                        ...
                                        <button onClick={() => handleRemoveQualification(i)} className="text-red-400 hover:text-red-500 p-1">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                ))}
                                {(!editForm.qualifications || editForm.qualifications.length === 0) && (
                                    <p className="text-slate-400 text-sm italic text-center">No qualifications listed.</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <ImageIcon size={20} className="text-purple-500" /> Clinic Gallery & Banner
                            </h3>
                            
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-gray-500 mb-2">Cover Banner URL (Full Width)</label>
                                <input 
                                    type="text" 
                                    placeholder="https://... (Cover Image)" 
                                    value={editForm.bannerUrl || ''}
                                    onChange={(e) => setEditForm({...editForm, bannerUrl: e.target.value})}
                                    className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-3 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                />
                            </div>
                            ...
                            <div className="flex gap-2 mb-4">
                                <input 
                                    ...
                                    className="flex-1 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-3 font-bold text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                                />
                                <button onClick={handleAddImage} className="bg-purple-600 hover:bg-purple-500 text-white p-3 rounded-xl transition-colors">
                                    <Plus size={20} />
                                </button>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {editForm.images?.map((img, i) => (
                                    <div key={i} className="relative group rounded-xl overflow-hidden aspect-square border border-gray-200 dark:border-slate-700">
                        ...
                        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700">
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer ${editForm.available ? 'bg-emerald-500' : 'bg-gray-300'}`} onClick={() => setEditForm({...editForm, available: !editForm.available})}>
                                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${editForm.available ? 'left-1' : 'right-1'}`}></div>
                                </div>
                                <span className="font-bold text-gray-900 dark:text-white">Available for booking</span>
                            </div>
                            <button 
