...
import { Doctor, Appointment, Product, Order, BlogPost, ForumTopic, AdoptionPet, User } from '../types';
...
export const api = {
        ...
        login: async (credentials: any) => {
            try {
                ...
                return new Promise((resolve) => setTimeout(() => resolve({
                    _id: 'u_' + Date.now(),
                    name: credentials.email.includes('admin') ? 'Super Admin' : 'Test User',
    ...
    users: {
        getAll: async (): Promise<User[]> => {
            try {
                ...
                return [{ id: 'u1', name: 'Super Admin', email: 'admin@vetcare.com', role: 'admin' as any, joinDate: '2023-01-01', status: 'active' }];
            }
        }
    },
    doctors: {
    ...
    appointments: {
        ...
        getForDoctor: async (doctorId: string) => {
            try {
                const res = await fetchWithTimeout(`${API_URL}/appointments/doctor/${doctorId}`);
                if (!res.ok) throw new Error();
                const data = await res.json();
                return data.map((a: any) => ({ ...a, id: a._id }));
            } catch (error) {
